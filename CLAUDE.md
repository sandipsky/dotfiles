...
- **Versioning.** The fork is `50.2.2-1.1` — a pkgrel bump over the repo's `-1` so `pacman -U` supersedes it. Both install.sh and rebuild-nautilus.sh ensure `IgnorePkg = nautilus libnautilus-extension` in `/etc/pacman.conf` so `-Syu` never replaces the fork; upgrades happen only by bumping the vendored tree. Keep the repo `nautilus` in install.sh's pacman list — it exists to pull in runtime deps before the fork overwrites it. To return to stock: remove the IgnorePkg line, `sudo pacman -S nautilus libnautilus-extension`.
...
- **Extensions still apply.** `nautilus-open-any-terminal` (AUR) and code-nautilus are separate extensions installed by install.sh — features that can be done as a nautilus-python extension don't need to be patched into the fork.
