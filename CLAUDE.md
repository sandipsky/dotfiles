...
What it does, in order: installs Hyprland/etc. packages, installs AUR packages via `yay` — including `noctalia-shell`, which pulls in `noctalia-qs`, a pinned Quickshell fork that conflicts with the official `quickshell` package, so never add `quickshell` to the pacman list — copies the udev rules from [assets/](assets/) into `/etc/udev/rules.d/` (substituting `$USERNAME`), copies font assets into `/usr/share/fonts`, installs `.desktop` overrides into the user's `~/.local/share/applications/`, copies [config/](config/) into the user's `~/.config/`, substitutes `USERNAME` into [config/noctalia/settings.json](config/noctalia/settings.json), seeds the user's avatar at `~/.face` from [assets/profile.png](assets/profile.png), sets up tty1 autologin + a `.zprofile` that `exec uwsm start hyprland.desktop`, sets gsettings + xdg-mime defaults, runs `xdg-user-dirs-update`, then reboots.
...
- [config/](config/) — copied verbatim to `~/.config/` (`alacritty`, `gtk-3.0`, `hypr`, `noctalia`, `systemd/user`). The bulk of the repo.
...
The shell is [Noctalia](https://github.com/noctalia-dev/noctalia) **v4** (the Quickshell-based line), installed from AUR as `noctalia-shell` into `/etc/xdg/quickshell/noctalia-shell/` and launched with `qs -c noctalia-shell`. It provides the bar (top) + auto-hide dock, launcher (app search / clipboard history / calculator / emoji), control center, notification daemon (`org.freedesktop.Notifications`), session menu, PAM lock screen, idle management (ext-idle-notify), night light (via `wlsunset`), wallpaper management, and volume/brightness OSDs — which is why this repo has no hyprlock/hypridle/hyprsunset/wpaperd/dunst configs.

Points that matter when changing things:

- **Settings.** [config/noctalia/settings.json](config/noctalia/settings.json) is a *partial* settings file — Noctalia deep-merges it over its built-in defaults, so only deviations from defaults belong in it. It contains `/home/USERNAME/...` paths with a literal `USERNAME` token that install.sh `sed`s at install time — keep the token in the repo copy. The Settings GUI on the machine rewrites the full file at runtime; those changes don't flow back into the repo, and re-running install.sh overwrites them.
